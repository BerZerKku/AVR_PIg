...
void testProtocolModbus::testGetStartAddress() {
	...
	struct sData {
		uint16_t adr;		// начальный адрес
		uint16_t num;		// количество адресов
		uint8_t numb;		// кол-во байт в посылке
		uint8_t buf[40];	// посылка
	};
...
void testProtocolModbus::testTrResponse()
{
	...
	sData data[] = {
		...
		{TProtocolModbus::STATE_READ_OK, TProtocolModbus::STATE_WRITE, 8}, // готовый ответ
		{TProtocolModbus::STATE_WRITE_WAIT, TProtocolModbus::STATE_WRITE_WAIT, 0},
		{TProtocolModbus::STATE_WRITE_READY, TProtocolModbus::STATE_WRITE, 6},	// ответ будет без CRC
...
/**	Проверка функции подсчета времени прошедшего с момента приема последнего
 *	байта по УАПП.
 * 
 *	Полноценная проверка зависит от функции push(), а точнее для правильного
 *	определения наличия принятой посылки, кол-во байт в буфере должно быть не 
...
void testProtocolModbus::testPrepareResponse() {
	...
	struct sData {
		uint8_t numin;		// кол-во байт в посылке
		uint8_t bufin[20];	// посылка
		uint8_t numout;		// кол-во байт в подготовленной посылке
		uint8_t	bufout[50];	// заготовка ответа
	...
	for (uint16_t i = 0; i < (sizeof (data) / sizeof (data[0])); i++) {
		...
		// проверка данных подготовленных для передачи
		...
		// проверка состояния после подготовки данных
		TProtocolModbus::STATE state = tProtocolModbus.getState();
		...
		// проверка кол-ва байт подготовленных для передачи
...
void testProtocolModbus::testSendRegister() {
	...
	struct sData {
		bool state;			// флаг выполнения операции (false - ошибка)
		uint16_t adr;		// адрес регистра
		uint16_t val;		// значение регистра	
		uint8_t numin;		// кол-во байт в буфере до записи
		uint8_t	bufin[50];	// буфер до записи
		uint8_t numout;		// кол-во байт в буфере после записи
		uint8_t bufout[50];	// буфер после записи
	...
	for(uint8_t i = 0; i < (sizeof(data)/sizeof(data[0])); i++){
		...
		// проверка флага выполнения функции
		if (state != data[i].state) {
		...
		// проверка подготовленных для передачи данных
		for (uint8_t j = 0; j < data[i].numout; j++) {
			if (buf[j] != data[i].bufout[j]) {
...
void testProtocolModbus::testSendCoil() {
	...
	for(uint8_t i = 0; i < (sizeof(data)/sizeof(data[0])); i++){
		...
		// проверка для данных которые должны быть записаны
		for (uint8_t j = 0; j < data[i].numout; j++) {
			if (buf[j] != data[i].bufout[j]) {
