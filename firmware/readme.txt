...
---> 26.01.26
	В ВОЛС кольцо:
		- добавлен параметры кольца "Протокол кольца БЦК1(2)".	
			Передаётся командой 0x0A (в ВЧ это "Частота").
		- добавлены предупреждения "Ош. прот-ла БЦКх" 
		- добавлены записи в журнал "Ошибка прот-ла БЦКх"
...
Изменения:
---> 03.12.25
	В ВОЛС исправлено отображение журналов передатчика и приёмника
...
Изменения:
---> 10.11.25
	В журналы ВЧ, ОПТИКА и ОПТИКА Кольцо добавлены записи о блокировке передатчика.
	Новые записи в ОПТИКА и ОПТИКА Кольцо передаются в байте режима.
	В протокол связи с БСП добавлен байт состояния передатчика.
	В 101 протоколе данные из данных класса 2 перенесены в класс 1.
	В 101 протоколе не передаются флаги отсутствующих устройств, например, в передатчике не будут передаваться сигналы приёмника и защиты.
...
Изменения:
---> 19.12.24
	В К400 ВЧ и ОПТИКА добавлен параметр передатчика "Управление передатчиком", новое предупреждение и записи журнала событий.
	На главном экране отображается состояние SAC2.
...
Изменения:
---> 09.12.24
	В К400 добавлен параметр приёмника "Автоматическая деблокировка".
	Для параметра добавлена наовая команда приёмника 0x1E/9E.
	SAC2 включается дефайном.
...
Изменения:
---> 25.10.24
	В ОПТИКА Кольцо добавлено предупреждение передатчика "Вывод команд ПРД".
	На главном экране добавлен вывод состояние ключа SAC2.
...
Изменения:
---> 16.11.2021
	В журнале приёмника К400 исправлено отображение не корректного номера команды (заменяется на 0).
...
Изменения:
---> 27.04.21
	Параметр приемника "Задержка на фикс. ком" во всех устройствах сделно можно изменять от 0 до 10 мс.
	Параметр приемника "Сигнализация команд" зависит от текущего количества команд приемника.
	В журнале вывод источника команды приемника оставлен только в 3-х концевом РЗСК
	Выбор источника синхронизации параметром "Синхронизация времени" оставлен только в Р400м, в остальных устройствах можно только включить или выключить.
