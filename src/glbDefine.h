...
enum eGB_COM
{
    ...
    GB_COM_GET_FREQ             = 0x3A,  // + ! в ВЧ - частота, в кольце - повышение безопасности
...
public:
     ...
     *
     *	1.53
     *	- Байт с дополнительными записями вынесен в отдельно, чтобы
     *	исправить работу журналов приёмника и передатчика.
     *
     *  1.52
     ...
     *  @retval True - всегда.
     */
    bool setOpticEntry(uint8_t *buf, uint8_t dop)
    {
        uint8_t num = 0;
            ...
            for (uint_fast8_t i = 0; i < size; i++)
            {
                ...
                for (uint_fast8_t j = 0; j < 8; j++)
                {
                    if (byte & (1 << j))
                    {
                        if (--num == 0)
                        {
                            // номер установленного бита, от 1 до 40
                            val = 1 + ((i * 8) + j);
                        }
                    }
                }
     ...
     * *************************************************************************
     *
     * @brief Устанавливает номер команды записанный в журнале.
     *
     * В случае выхода номера команды за диапазон, номер будет сброшен в 0.
     *
     * @param[in] num Команда [1..99].
    ...
    {
        // ограничение - два символа, команда 0 считается ошибкой
        if (num >= 100)
        {
            num = 0;
        }
     ...
     * *************************************************************************
     *
     * @brief Возвращает номер команды записанный в журнале.
    ...
    /**
     * *************************************************************************
     * @brief Установливает источник команды передатчика.
     *
     * В случае не корректного значения, в параметр будет записано значение
     * \a GB_SOURCE_COM_MAX.
     *
     * @param[in] source Источник передаваемой команды.
     * @return true если значение корректное, иначе false.
     *
     *  *************************************************************************
     */
    ...
    {
        if (source > GB_SOURCE_COM_MAX)
        {
            source = GB_SOURCE_COM_MAX;
        }
        sourceCom_ = source;

        return source != GB_SOURCE_COM_MAX;
    }

    /**
     * *************************************************************************
     * @brief Возвращает источник команды передатчика.
     * @return Источник передаваемой команды.
     * @retval GB_SOURCE_COM_MAX в случае ошибки.
     *
     *  *************************************************************************
     */
