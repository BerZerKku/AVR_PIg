...
bool TUart::open(eUART_BAUD_RATE baud, eUART_DATA_BITS databits,
	...
	// дл€ округлени€ в большую сторону добавим 4*baudrate
	ibaud = ((F_CPU + 4*ibaud) / (8*ibaud)) - 1;
	ucsra |= (1 << U2X);					// вкл. удвоени€ скорости работы
...
void TUart::close() {
	// запрет работы передатчика/приемника и прервани€ по приему
	*ucsrb_ &= ~((1 << TXEN1) | (1 << RXEN1) | (1 << RXCIE1));
}
...
/**	ќтправка заданного кол-ва данных, заранее помещенных в буфер
 * 	@param size  ол-во байт данных
 * 	@return  ол-во отправл€емых байт данных
 */
uint8_t TUart::trData(uint8_t size) {
	if (size != 0) {
		...
		*udr_ = buf_[0];
		...
		// –азрешим прерывание по опустошению буфера UART и окончанию передачи
