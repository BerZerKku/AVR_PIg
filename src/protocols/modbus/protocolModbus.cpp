...
// Конструктор
TProtocolModbus::TProtocolModbus(uint8_t *buf, uint8_t size) : buf_(buf), size_(size)
{
    state_   = STATE_OFF;
...
bool TProtocolModbus::readData()
{
    ...
    if (checkReadPackage() == CHECK_ERR_NO)
    {
        ...
        switch (com)
        {
        case COM_01H_READ_COILS: state = comReadCoils(); break;
        case COM_02H_READ_DISCRETE_INPUTS: state = comReadDInputs(); break;
        case COM_03H_READ_HOLDING_REGISTERS: state = comReadRegisters(); break;
        case COM_04H_READ_INPUT_REGISTERS: state = comReadIRegisters(); break;
        case COM_05H_WRITE_SINGLE_COIL: state = comWriteCoil(); break;
        case COM_06H_WRITE_SINGLE_REGISTER: state = comWriteRegister(); break;
        case COM_0FH_WRITE_MULTIPLIE_COILS: state = comWriteCoils(); break;
        case COM_10H_WRITE_MULITPLIE_REGISTERS: state = comWriteRegisters(); break;
        case COM_11H_SLAVE_ID: state = comReadID(); break;
        default:
