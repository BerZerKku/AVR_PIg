 ...
 */
class CIec101 {
	/// Пауза между принятыми байтами, по которой определяется конец принятой посылки.
	static const uint16_t s_u16DelayFinish = 15000;	// лучше кратно 1500
	/// Минимальный интервал между кадрами при ошибки в формате с переменным числом пользовательских данных
	static const uint16_t s_u16DelayFix = 30000; 	// лучше кратно 50
	/// Минимальный интервал между кадрами при ошибке в формате с фиксированным числом пользовательсних данных
	static const uint16_t s_u16DelayVar = 20000; 	// лучше кратно 50
...
//		FUNCTION_INTERROG		= 0x0C,	///< \b <0x0C> Наличие опроса (общее).
	 ...
	 *
	 * 	step = (\a u8Period / 10^6) * (#s_u16DelayFinish / 1.5) * (\a u16Baudrate / 11) =
	 ...
	 *	спокойного состояния \a #s_u16DelayFix, а затем переход в \a #STATE_READ
	 *	(т.е. ожидание новой посылки).
	 *
	 ...
	 *	\a #STATE_WRITE_READY.
	 *
	 * 	@return Код ошибки, если она обнаружена (\a #EError).
	 * 	@retval 0 - ошибок нет.
	 */
	CIec101::EError readData();
	 ...
	 *
	 * 	В функцию передается текущее время, которое будет изменено при необходимости
	 * 	синхронизации. При этом будет возвращено \a True. Если времени для
	 ...
	 * 	- UI8[1..8]<0..99> год;
	 * 	- UI8[1..8]<1..12> месяц;
	 * 	- UI8[1..8]<1..31> день месяца;
	 * 	- UI8[1..8]<0..23> часы;
	 * 	- UI8[1..8]<0..59> минуты;
	 * 	- UI8[1..8]<0..59> секунды.
	 *
	 * 	В данной функции, массив откуда копируется имеет вид:
	 * 	- UI16[1..16]<0> миллисекунды (младшим байтом вперед);
	 * 	- UI8[1..8]<0..59> секунды;
	 * 	- UI8[1..8]<0..59> минуты;
	 * 	- UI8[1..8]<0..23> часы;
	 * 	- UI8[1..8]<0> день недели;
	 * 	- UI8[1..8]<1..31> день месяца;
	 * 	- UI8[1..8] <1..12> месяц;
	 * 	- UI8[1..8]<0..99> год.
	 *
	 *	@param[out] pDist Куда копируется новое время.
	 *	@param[in] pSource Откуда копируется текущее время.
	 ...
	 *
	 *	В данной функции, при вызове просто устанавливается флаг наличия данных
	 *	на передачу.
	 *
