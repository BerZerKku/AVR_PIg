...
void clMenu::lvlControl()
{
	...
	// в зависимости от текущего состояния выведем строку
...
void clMenu::lvlSetupParamDef()
{
	...
	if (enterParam.isEnable())
		...
		else
		{
			...
			{
				snprintf_P(&vLCDbuf[poz], 11,
						fcPvzlFreq[sParam.def.getFreqPrd()]);
			}
			else if (p == punkt11)
			{
				snprintf_P(&vLCDbuf[poz], 11,
						fcPvzlFreq[sParam.def.getFreqPrm()]);
			}
		}
	switch (key_)
	{
	...
	switch (key_)
	{
		...
		case KEY_ENTER:
			...
			else
			{
				...
				{
					enterParam.setEnable();
					enterParam.setValueRange(PRM_TIME_OFF_MIN,
							PRM_TIME_OFF_MAX);
	...
	switch (key_)
	{
		...
		case KEY_ENTER:
			...
			else
			{
				...
				{
					enterParam.setEnable();
					enterParam.setValueRange(PRD_DURATION_MIN,
							PRD_DURATION_MAX);
...
//				sParam.txComBuf.addCom(GB_COM_GET_TIME_RERUN);
	...
	{
		snprintf_P(&vLCDbuf[poz], 11, fcRangeDec, GLB_FREQ_MIN,
				GLB_FREQ_MAX, "кГц");
	}
	else if (p == punkt12)
	{
	...
	else
	{
		...
		{
			snprintf(&vLCDbuf[poz],11,"%uград", sParam.glb.getPvzueNoiseLvl());
		}
		else if (p == punkt22)
		{
	...
	switch (key_)
	{
		...
		case KEY_ENTER:
			...
			{
				enterParam.setEnable();
				enterParam.setValueRange(GLB_CF_THRESH_MIN,
						GLB_CF_THRESH_MAX);
			...
			{
				enterParam.setEnable();
				enterParam.setValueRange(GLB_U_OUT_NOM_MIN,
						GLB_U_OUT_NOM_MAX);
			...
			{
				enterParam.setEnable();
				enterParam.setValueRange(GLB_AC_IN_DEC_MIN,
						GLB_AC_IN_DEC_MAX);
			...
			{
				enterParam.setEnable();
				enterParam.setValueRange(GLB_PVZUE_FAIL_MIN,
						GLB_PVZUE_FAIL_MAX);
			...
			{
				enterParam.setEnable();
				enterParam.setValueRange(GLB_PVZUE_N_TH_MIN,
						GLB_PVZUE_N_TH_MAX);
			...
			{
				enterParam.setEnable();
				enterParam.setValueRange(GLB_PVZUE_N_LVL_MIN,
						GLB_PVZUE_N_LVL_MAX);
