...
public:
	 ...
	 *
	 * 	Флаг ошибки сбрасывается, т.к. нет смысла проверять битовую
	 * 	переменную.
	 *
	 * 	@param *ptr Указатель на первый байт данных.
	 */
	void setValueBits(uint8_t *ptr);
...
//	const Param* param[MAX_NUM_OF_PARAMS];	///< Массив параметров.
	...
	uint8_t valB[4];		///< Значение для битовых переменных.
