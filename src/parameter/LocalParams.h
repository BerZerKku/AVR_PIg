...
class LocalParams {
	...
	/// Максимальное количетсов байт для битовых переменных.
...
public:
	 ...
	 *
	 *	Если в одном байте содержутся биты данных для разных параметров
	 *	(\a Param::PARAM_BITE), то возвращается значение бит, кол-во которых
	 *	определяется по максимуму.
	 *
	...
	* 	Если параметр был изменен, то устаналивается флаг обновления \a refresh.
	...
	/**	Возвращает тип параметра для чтения значения.
	 *
	 * 	@return Тип параметра для чтения значения.
	 */
	...
	/**	Возвращает положение первого бита для параметров \a Param::PARAM_BITE.
	 *
	 * 	@return Положение первого бита.
	 */
	uint8_t getPosBite() const {
		return pgm_read_byte(&getPtrParam()->posBite);
	}

	/**	Установка значения неоднородного битового параметра \a Param::PARAM_BITE.
	 *
	 *	@param nval[in] Новое значение текущего неоднородного параметра.
	 * 	@return Возвращает байт данных с новым значением параметра.
	 */
...
private:
	...
	/**	Возвращает маску для неоднородных битовых параметров \a Param::PARAM_BITE.
	 *
	 * 	@return Маска для неоднородного битового параметра.
	 */
