...
#include "ks0108.h"
#include "menu.h"
// \fixme Если поставить библиотеки в алфавитном порядке появляются ошибки компиляции
#include "flash.h"
    ...
    case KEY_AC_PUSK:
        if (sParam.def.status.isEnable())
        {
            if (sParam.typeDevice == AVANT_R400M)
            {
                ...
                if (comp != GB_COMPATIBILITY_LINER)
                {
                    if ((comp == GB_COMPATIBILITY_AVZK80) || (comp == GB_COMPATIBILITY_PVZ90))
                    {
...
void clMenu::lvlFirst()
{
    ...
    case KEY_ENTER:
        ...
        case 5:
            ...
            lvlCreate_ = true;
            break;
        case 6: lvlMenu = &clMenu::lvlMeasure; lvlCreate_ = true;
        }
        break;
    ...
    else
    {
        ...
        else if (device == AVANT_K400)
        {
            if (event <= MAX_JRN_EVENT_VALUE)
            {
                ...
                snprintf_P(&vLCDbuf[poz], 21, fcJrnEventK400[event], fcDevicesK400[dev]);
            }
            else
            {
        ...
        else if (device == AVANT_RZSK)
        {
            ...
            {
                uint8_t dev = (uint8_t) sParam.jrnEntry.getDeviceJrn();
                snprintf_P(&vLCDbuf[poz], 21, fcJrnEventRZSK[event], fcDevicesK400[dev]);
            }
            else
            {
...
void clMenu::lvlTest2()
{
    ...
    if ((sParam.def.getNumDevices() == GB_NUM_DEVICES_3) && (sParam.typeDevice != AVANT_R400M))
    {
        ...
        snprintf_P(&vLCDbuf[poz], 11, fcTest1K400[sParam.test.getCurrentSignal2()]);
    }
    else
    {
        poz = 100;
...
eMENU_ENTER_PARAM clMenu::enterValue()
{
    ...
    else if (status == MENU_ENTER_PARAM_LIST_2)
    {
        ...
        clearLine(NUM_TEXT_LINES);
        uint8_t poz = 100;
        snprintf_P(&vLCDbuf[poz], 21, enterList, EnterParam.list + STRING_LENGHT * val);
    }
    else
        key_ = KEY_CANCEL;
...
void clMenu::printMeasParam(uint8_t poz, eMENU_MEAS_PARAM par)
{
    ...
    if (poz < 12)
    {
        ...
        case MENU_MEAS_PARAM_UZ1:
            snprintf_P(&vLCDbuf[poz], 11, fcUz1, sParam.measParam.getVoltageDef());
            break;
        case MENU_MEAS_PARAM_UZ2:
            snprintf_P(&vLCDbuf[poz], 11, fcUz2, sParam.measParam.getVoltageDef2());
            break;
        ...
        case MENU_MEAS_PARAM_UC1:
            snprintf_P(&vLCDbuf[poz], 11, fcUcf1, sParam.measParam.getVoltageCf());
            break;
        case MENU_MEAS_PARAM_UC2:
            snprintf_P(&vLCDbuf[poz], 11, fcUcf2, sParam.measParam.getVoltageCf2());
            break;
        ...
        case MENU_MEAS_PARAM_UN1:
            snprintf_P(&vLCDbuf[poz], 11, fcUn1, sParam.measParam.getVoltageNoise());
            break;
        case MENU_MEAS_PARAM_UN2:
            snprintf_P(&vLCDbuf[poz], 11, fcUn2, sParam.measParam.getVoltageNoise2());
            break;
...
void clMenu::printDevicesStatus(uint8_t poz, TDeviceStatus *device)
{
    ...
    else
    {
        ...
        snprintf_P(&vLCDbuf[poz], 9, text[device->getState()], device->getDopByte());
    }
}
...
void clMenu::printRange(uint8_t pos)
{
    ...
    {
    case Param::RANGE_LIST: str = PSTR("список"); break;

    case Param::RANGE_ON_OFF: str = PSTR("вкл./выкл."); break;

    case Param::RANGE_INT: str = PSTR("%d..%d%S"); break;

    case Param::RANGE_INT_NO_DIM: str = PSTR("%d..%d"); break;
