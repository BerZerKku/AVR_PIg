...
TEST_F(ProtocolModbusTest_Push_and_Tick, tick)
{
    ...
    sData data[] = { { TPM::STATE_OFF, TPM::STATE_OFF, 8, 19200, 50, 50 },
...
TEST_F(ProtocolModbusTest_Push_and_Tick, push)
{
    ...
    sData data[] = { { TPM::STATE_OFF, TPM::STATE_OFF, 20, 21, 0 },
...
TEST_F(ProtocolModbusTest_AddressLan, get_and_set)
{
    ...
    for (uint16_t i = 0; i <= 255; i++)
    {
        ...
        if (i < adrMin)
        {
            ...
            {
                sprintf(msg,
                        "  >>> Значение меньше ADDRESS_MIN не ведет к ADDRESS_ERR на шаге %d",
                        i);
                ASSERT_TRUE(false) << msg;
            }
        }
        else if (i > adrMax)
        {
            ...
            {
                sprintf(msg,
                        "  >>> Значение больше ADDRESS_MAX не ведет к ADDRESS_MAX на шаге %d",
                        i);
                ASSERT_TRUE(false) << msg;
            }
        }
...
public:
    ...
    // деструктор
    virtual ~ProtocolModbusTest_readData() { delete mb; }
...
TEST_F(ProtocolModbusTest_readData, com_0x01_read_coil)
{
    ...
    uint8_t res5[] = { 0x11, 0x01, 0x20, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
...
TEST_F(ProtocolModbusTest_readData, com_0x02_discrete_inputs)
{
    ...
    uint8_t res5[] = { 0x11, 0x02, 0x20, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
                       0xFF, 0xFF, 0xFF, 0x0F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                       0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };
    ASSERT_TRUE(test(req5, SIZE_ARRAY(req5), res5, SIZE_ARRAY(res5))) << msg;
...
TEST_F(ProtocolModbusTest_readData, com_0x04_read_input_register)
{
    ...
    uint8_t res4[] = {
        0x01, 0x04, 0x0A, 0x00, 0x61, 0x00, 0x62, 0x00, 0x63, 0x00, 0x64, 0xFF, 0xFF
    };
    ASSERT_TRUE(test(req4, SIZE_ARRAY(req4), res4, SIZE_ARRAY(res4))) << msg;

    // считывание максимального количества адресов
    uint8_t req5[] = { 0x01, 0x04, 0x00, 0x50, 0x00, 0x20, 0xF1, 0xC3 };
    uint8_t res5[] = { 0x01, 0x04, 0x40, 0x00, 0x51, 0x00, 0x52, 0x00, 0x53, 0x00, 0x54, 0x00,
                       0x55, 0x00, 0x56, 0x00, 0x57, 0x00, 0x58, 0x00, 0x59, 0x00, 0x5A, 0x00,
                       0x5B, 0x00, 0x5C, 0x00, 0x5D, 0x00, 0x5E, 0x00, 0x5F, 0x00, 0x60, 0x00,
                       0x61, 0x00, 0x62, 0x00, 0x63, 0x00, 0x64, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
                       0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
...
TEST_F(ProtocolModbusTest_readData, com_0x0F_write_multiplie_coils)
{
    ...
    uint8_t req5[] = { 0x01, 0x0F, 0x00, 0x00, 0x01, 0x00, 0x20, 0x01, 0x02, 0x03, 0x04,
                       0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0xF0, 0xFF, 0xFF,
                       0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x00,
                       0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x8B, 0x8A };
    ...
    // исключение EXCEPTION_03H_ILLEGAL_DATA_VAL
    uint8_t req11[] = { 0x11, 0x0F, 0x00, 0x13, 0x01, 0x00, 0x21, 0x01, 0x01, 0x01, 0x01,
                        0x01, 0x01, 0x01, 0x01, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02,
                        0x02, 0x03, 0x03, 0x03, 0x03, 0x03, 0x03, 0x03, 0x03, 0x04, 0x04,
                        0x04, 0x04, 0x04, 0x04, 0x04, 0x04, 0x05, 0xA7, 0x9E };
...
TEST_F(ProtocolModbusTest_readData, com_0x10_write_multiplie_registers)
{
    ...
    uint8_t req2[] = { 0x01, 0x10, 0x00, 0x10, 0x00, 0x07, 0x0E, 0x15, 0x0F, 0x14, 0x88, 0x91,
                       0x94, 0x71, 0x98, 0x12, 0xFF, 0xE1, 0x12, 0x17, 0x71, 0xEA, 0x36 };
    uint8_t res2[] = { 0x01, 0x10, 0x00, 0x10, 0x00, 0x07 };
    ASSERT_TRUE(test(req2, SIZE_ARRAY(req2), res2, SIZE_ARRAY(res2))) << msg;

    // проверка записи группы регистров, больше 8
    uint8_t req3[] = { 0x01, 0x10, 0x00, 0x25, 0x00, 0x09, 0x12, 0x15, 0x0F,
                       0x14, 0x88, 0x91, 0x94, 0x71, 0x98, 0x12, 0xFF, 0xE1,
                       0x12, 0x17, 0x71, 0x77, 0x99, 0x13, 0x14, 0x0A, 0x27 };
    uint8_t res3[] = { 0x01, 0x10, 0x00, 0x25, 0x00, 0x09 };
    ASSERT_TRUE(test(req3, SIZE_ARRAY(req3), res3, SIZE_ARRAY(res3))) << msg;

    // проверка записи группы регистров, кратно 8
    uint8_t req4[] = { 0x01, 0x10, 0x00, 0xC6, 0x00, 0x08, 0x10, 0x00, 0x11, 0x00, 0x62, 0x91, 0x94,
                       0x71, 0x98, 0x12, 0xFF, 0xE1, 0x12, 0x17, 0x71, 0x77, 0x99, 0x0B, 0x51 };
    uint8_t res4[] = { 0x01, 0x10, 0x00, 0xC6, 0x00, 0x08 };
    ASSERT_TRUE(test(req4, SIZE_ARRAY(req4), res4, SIZE_ARRAY(res4))) << msg;

    // проверка записи максимального кол-ва регистров, 32
    uint8_t req5[] = { 0x11, 0x10, 0x00, 0x01, 0x00, 0x20, 0x40, 0x01, 0x01, 0x01, 0x01,
                       0x01, 0x01, 0x01, 0x01, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02,
                       0x02, 0x03, 0x03, 0x03, 0x03, 0x03, 0x03, 0x03, 0x03, 0x04, 0x04,
                       0x04, 0x04, 0x04, 0x04, 0x04, 0x04, 0xED, 0xDE };
    ...
    // исключение EXCEPTION_02H_ILLEGAL_DATA_ADR
    uint8_t req6[] = {
        0x01, 0x10, 0x01, 0x2C, 0x00, 0x02, 0x04, 0xFF, 0xFF, 0xAA, 0xBB, 0xC2, 0x85
    };
    ...
    //  исключение EXCEPTION_02H_ILLEGAL_DATA_ADR
    uint8_t req7[] = { 0x01, 0x10, 0x01, 0x2A, 0x00, 0x04, 0x08, 0xFF, 0xFF,
                       0x10, 0x00, 0x51, 0x98, 0x78, 0xFF, 0x9C, 0x94 };
    ...
    // исключение EXCEPTION_04H_DEVICE_FAILURE
    uint8_t req8[] = {
        0x88, 0x10, 0x00, 0x7D, 0x00, 0x02, 0x04, 0x00, 0x01, 0x00, 0x65, 0x23, 0xA7
    };
    ...
    // исключение EXCEPTION_04H_DEVICE_FAILURE
    uint8_t req9[] = {
        0x88, 0x10, 0x00, 0xC8, 0x00, 0x02, 0x04, 0x03, 0xEE, 0x03, 0xE7, 0x59, 0xCC
    };
    ...
    // исключение EXCEPTION_03H_ILLEGAL_DATA_VAL
    uint8_t req11[] = { 0x11, 0x10, 0x00, 0x01, 0x00, 0x21, 0x42, 0x01, 0x01, 0x01, 0x01,
                        0x01, 0x01, 0x01, 0x01, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02,
                        0x02, 0x03, 0x03, 0x03, 0x03, 0x03, 0x03, 0x03, 0x03, 0x04, 0x04,
                        0x04, 0x04, 0x04, 0x04, 0x04, 0x04, 0x05, 0x05, 0x05, 0x05, 0x05,
                        0x05, 0x05, 0x05, 0x06, 0x06, 0x06, 0x06, 0x06, 0x06, 0x06, 0x06,
                        0x07, 0x07, 0x07, 0x07, 0x07, 0x07, 0x07, 0x07, 0x08, 0x08, 0x08,
                        0x08, 0x08, 0x08, 0x08, 0x08, 0x09, 0x09, 0x19, 0xEC };
...
public:
    ...
    // деструктор
    virtual ~ProtocolModbusTest_trResponse() { delete mb; }
...
TEST_F(ProtocolModbusTest_trResponse, trResponse)
{
    ...
    uint8_t req6[] = { 0x01, 0x0F, 0x00, 0x00, 0x01, 0x00, 0x20, 0x01, 0x02, 0x03, 0x04,
                       0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0xF0, 0xFF, 0xFF,
                       0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x00,
                       0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x8B, 0x8A };
    ...
    uint8_t res8[] = {
        0x03, 0x11, 0x09, 'V', 'i', 'r', 't', 'u', 'a', 'l', '\0', 0x00, 0xFB, 0x6D
    };
    ASSERT_TRUE(test(req8, SIZE_ARRAY(req8), res8, SIZE_ARRAY(res8))) << msg;
    ...
    uint8_t res11[] = { 0x01, 0x04, 0x0A, 0x00, 0x61, 0x00, 0x62, 0x00,
                        0x63, 0x00, 0x64, 0xFF, 0xFF, 0xB3, 0x4E };
    ASSERT_TRUE(test(req11, SIZE_ARRAY(req11), res11, SIZE_ARRAY(res11))) << msg;
}
