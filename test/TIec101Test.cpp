...
bool checkMass(const uint8_t mas[], uint8_t masSize, const uint8_t exp[], uint8_t expSize)
{
    ...
    {
        posMsg +=
            sprintf(&msg[posMsg], " Размер полученного массива %d, вместо %d", masSize, expSize);
        state = false;
    }
    else
    {
        for (uint8_t i = 0; i < masSize; i++)
        {
            ...
            {
                posMsg += sprintf(&msg[posMsg],
                                  " Ошибка байта mas[%d] = %02X, вместо %02X",
                                  i,
                                  mas[i],
                                  exp[i]);
                state = false;
...
TEST_F(TIec101_Full, adressLan)
{
    ...
    {
        posMsg += sprintf(&msg[posMsg], "  >>> Адрес по умолчанию не совпадает с ADDRESS_ERR.");
    ...
    for (uint16_t i = 0; i <= 255; i++)
    {
        ...
        {
            posMsg += sprintf(&msg[posMsg],
                              "  >>> setAddress() возвращает ошибочное состояние на шаге %d",
                              i);
            ASSERT_TRUE(false) << msg;
        }
        ...
        {
            posMsg += sprintf(&msg[posMsg],
                              "  >>> Установленный адрес не совпадает со считанным на шаге %d",
                              i);
            ASSERT_TRUE(false) << msg;
        }
    }
...
TEST_F(TIec101_Full, push)
{
    ...
    for (uint16_t i = 1; i <= 2 * sizeof(buf); i++)
    {
        ...
        if (i <= sizeof(buf))
        {
            ...
            {
                posMsg +=
                    sprintf(&msg[posMsg], "  <<< Неверное количество принятых байт на шаге %d", i);
                ASSERT_TRUE(false) << msg;
            }
        }
        else
        {
            if (t != sizeof(buf))
            {
                posMsg += sprintf(&msg[posMsg], "  <<< Переполнение буфера на шаге %d", i);
                ASSERT_TRUE(false) << msg;
            }
        }
    }
    ...
    sData data[] = {
        { CIec101::STATE_OFF, 10, 11, 0 },     { CIec101::STATE_OFF, 10, 5, 0 },
        { CIec101::STATE_READ, 10, 10, 10 },   { CIec101::STATE_READ, 10, 18, 10 },
        { CIec101::STATE_READ, 10, 2, 2 },     { CIec101::STATE_READ_ERROR, 10, 18, 0 },
        { CIec101::STATE_READ_OK, 10, 18, 0 }, { CIec101::STATE_WRITE_WAIT, 10, 18, 0 },
    ...
    for (uint16_t i = 0; i < (sizeof(data) / sizeof(data[0])); i++)
    {
        ...
        {
            posMsg += sprintf(&msg[posMsg], "  >>> Ошибочное кол-во принятых байт на шаге %d", i);
...
TEST_F(TIec101_Full, getNumOfBytes)
{
    ...
    for (uint8_t i = 0; i < SIZE; i++)
    {
        ...
        if ((i + 1) != iec101->getNumOfBytes())
        {
            posMsg += sprintf(&msg[posMsg], "  >>> Ошибка на шаге %d", i);
            posMsg +=
                sprintf(&msg[posMsg], "\n num = %d, need = %d", iec101->getNumOfBytes(), i + 1);
            ASSERT_TRUE(false) << msg;
        }
    }
    ...
    for (uint8_t i = 0; i < SIZE; i++)
    {
        ...
        if (SIZE != iec101->getNumOfBytes())
        {
            posMsg += sprintf(&msg[posMsg], "  >>> Ошибка на шаге %d", i);
            posMsg +=
                sprintf(&msg[posMsg], "\n num = %d, need = %d", iec101->getNumOfBytes(), SIZE);
            ASSERT_TRUE(false) << msg;
        }
    }
}
...
TEST_F(TIec101_Full, readData_Var_check)
{
    ...
    {
        printTestName(" Ошибочное поле управления. \n");
        uint8_t mas[] = { FRAME_START_CHARACTER_VAR,
                          0x09,
                          0x09,
                          FRAME_START_CHARACTER_VAR,
                          0x33,
                          0x01,  // control + link address
                          0x64,
                          0x01,  // address (2 bytes) + siq
                          0x06,
                          0x01,
                          0x00,
                          0x00,
...
TEST_F(TIec101_Full, readData_reset)
{
    ...
    // Проверка должна проводиться на корректных запросах!
    {
        ...
        uint8_t mas[] = { FRAME_START_CHARACTER_VAR,
                          ...
                          FRAME_START_CHARACTER_VAR,
                          0x70 + 0,
                          0x01,  // control + link address RESET_REMOTE_LINK
                          0x00,
                          0x01,
                          0x06,  // address (2 bytes) + siq
                          0x01,
                          0x00,
                          0x00,
                          0x14,
...

    {
        ...
        uint8_t mas[] = { FRAME_START_CHARACTER_VAR,
                          ...
                          FRAME_START_CHARACTER_VAR,
                          0x70 + 3,
                          0x01,  // control + link address USER_DATA_CONFIRM
                          0x00,
                          0x01,
                          0x06,  // address (2 bytes) + siq
                          0x01,
...

    {
        ...
        uint8_t mas[] = { FRAME_START_CHARACTER_VAR,
                          ...
                          FRAME_START_CHARACTER_VAR,
                          0x70 + 4,
                          0x01,  // control + link address USER_DATA_NO_REPLY
                          0x00,
                          0x01,
                          0x06,  // address (2 bytes) + siq
                          0x01,
...

    {
        ...
        uint8_t mas[] = { FRAME_START_CHARACTER_VAR,
                          ...
                          FRAME_START_CHARACTER_VAR,
                          0x70 + 9,
                          0x01,  // control + link address REQUEST_STATUS_OF_LINK
                          0x00,
                          0x01,
                          0x06,  // address (2 bytes) + siq
                          0x01,
...

    {
        ...
        uint8_t mas[] = { FRAME_START_CHARACTER_VAR,
                          ...
                          FRAME_START_CHARACTER_VAR,
                          0x70 + 10,
                          0x01,  // control + link address REQUEST_USER_DATA_CLASS_1
                          0x00,
                          0x01,
                          0x06,  // address (2 bytes) + siq
                          0x01,
...

    {
        ...
        uint8_t mas[] = { FRAME_START_CHARACTER_VAR,
                          ...
                          FRAME_START_CHARACTER_VAR,
                          0x70 + 11,
                          0x01,  // control + link address REQUEST_USER_DATA_CLASS_2
                          0x00,
                          0x01,
                          0x06,  // address (2 bytes) + siq
                          0x01,
...
TEST_F(TIec101_Full, readData_TimeSynch)
{
...

    {
        ...
        uint8_t req[] = {
            ...
            FRAME_START_CHARACTER_VAR,
            (uint8_t) (0x43
                       + getFcb(false)),  // control = USER_DATA_CONFIRM, fcb = 1, fcv = 1, prm = 1
            0x01,                         // link address
        ...
        ASSERT_EQ(iec101->checkFrame(), CIec101::ERROR_NO)
            << msg << " Кол-во байт: " << (uint16_t) iec101->getNumOfBytes();
        iec101->readData();
...

    {
        ...
        uint8_t req[]  = { FRAME_START_CHARACTER_VAR,
                          0x0F,
                          0x0F,
                          FRAME_START_CHARACTER_VAR,
        ...
        ASSERT_TRUE(checkMass((uint8_t *) &iec101->stTime,
                              sizeof(iec101->stTime),
                              time_exp,
                              sizeof(time_exp)))
            << msg;
    }
    ...
    //		ASSERT_TRUE(checkMass(tmp_exp, sizeof(tmp_exp), time_exp , sizeof(time_exp))) <<
    //msg;
    //	}
...
TEST_F(TIec101_Full, readData_Iterrog)
{
...

    {
        ...
        uint8_t req[] = {
            ...
            FRAME_START_CHARACTER_VAR,
            (uint8_t) (0x43
                       + getFcb(true)),  // control = USER_DATA_CONFIRM, fcb = 0, fcv = 1, prm = 1
            0x01,                        // link address
...
TEST_F(TIec101_Full, readData_Events_Class1)
{
    ...
    {
        printTestName(
            " Отсутсвие данных класса 1 на передачу (RESPOND_NACK + ack=0 в команде класса 2). \n");
    ...
    {
        printTestName(
            " Отсутсвие данных класса 1 на передачу (RESPOND_NACK + ack=0 в команде класса 1). \n");
        uint8_t req[]  = FIX_FRAME_IN(11, true);
        uint8_t resp[] = { FRAME_START_CHARACTER_FIX, 0x09, 0x01, 0x0A, FRAME_STOP_CHARCTER };
